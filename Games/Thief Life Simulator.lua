            ...
            if (autoFarmWindow.flags.autoFarmToggle) then
                for _,LootPlace in pairs(game.Workspace.Loot.Places:GetChildren()) do
                    if (LootPlace.LootType.Value == 'HighValueLoot') then
                        game.ReplicatedStorage.Events.Loot.RequestLoot:FireServer(LootPlace.Name)
                        for _, Loot in pairs((game.Players.LocalPlayer.PlayerGui.LootGUI.LootWindow.Loot:GetChildren())) do
                            if Loot.Name ~= "UIListLayout" then
                                game.ReplicatedStorage.Events.Loot.RequestPickUp:FireServer(Loot.Name)
                                game:GetService('SoundService').Pickup:Play()
                            end
                        end
                    end
                end
