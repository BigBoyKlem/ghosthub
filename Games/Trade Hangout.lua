...
if (checkWhitelist()) then
    ...
    mainWindow:Box('Message', {flag = 'autoChatMessage'})
    mainWindow:Box('Delay (Seconds)', {flag = 'autoChatDelay', type = 'number'})
    mainWindow:Toggle('Auto Chat', {flag = 'autoChatToggle'})
    ...
    spawn(
        function()
            while true do
                if (mainWindow.flags.autoChatToggle) then
                    game.ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(mainWindow.flags.autoChatMessage,"All")
                    wait(mainWindow.flags.autoChatDelay)
                else
                    wait()
                end
            end
